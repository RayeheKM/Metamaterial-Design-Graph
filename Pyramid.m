function [Stiffness, Impedance, VolumeFraction] = Pyramid(NameData, Grid_Number, cube_size, u_disp, rho, E, nu)
    ...
    ratio = 0;  % it can change a pyramid to a truncated pyramid

    name_of_model=[NameData,'.mph'];
    nodes=load([Directory_files,NameData,'.mat']);

    Number_of_Pyramids=length(nodes.pyramids);
    CornerNodes=zeros(Number_of_Pyramids,15);
    CornerNodes(:,:)=nodes.pyramids(:,:);
    SquareLength=round(norm(CornerNodes(1,4:6)-CornerNodes(1,7:9)),5);
    ...
    Avg_Square=zeros(1,3);
    for i=1:Number_of_Pyramids
        Avg_Square(1)=round((CornerNodes(i,4)+CornerNodes(i,7)+CornerNodes(i,10)+CornerNodes(i,13))/4, 5);
        Avg_Square(2)=round((CornerNodes(i,5)+CornerNodes(i,8)+CornerNodes(i,11)+CornerNodes(i,14))/4, 5);
        Avg_Square(3)=round((CornerNodes(i,6)+CornerNodes(i,9)+CornerNodes(i,12)+CornerNodes(i,15))/4, 5);

        Axis=round([CornerNodes(i,1),CornerNodes(i,2),CornerNodes(i,3)]-Avg_Square, 5);

        model.component('comp1').geom('geom1').create(['pyr',num2str(i)], 'Pyramid');
        model.component('comp1').geom('geom1').feature(['pyr',num2str(i)]).set('a', SquareLength);
        model.component('comp1').geom('geom1').feature(['pyr',num2str(i)]).set('b', SquareLength);
        model.component('comp1').geom('geom1').feature(['pyr',num2str(i)]).set('h', SquareLength/2*(1-ratio));
        model.component('comp1').geom('geom1').feature(['pyr',num2str(i)]).set('rat', ratio);
        model.component('comp1').geom('geom1').feature(['pyr',num2str(i)]).set('pos', Avg_Square);
        model.component('comp1').geom('geom1').feature(['pyr',num2str(i)]).set('axistype', 'cartesian');
        model.component('comp1').geom('geom1').feature(['pyr',num2str(i)]).set('ax3', Axis);
    end
